...
public function duplicatebook($data){
    $givenName = strtolower(trim($data->book_title));
    $givenName = str_replace('_', '', $givenName);
    $existingName = Book::all();
    $arrdata = [];
    foreach($existingName as $key=>$val){
        $name = strtolower(trim($val->book_title));
