    ...
    <div id="main-wrapper">
        ...
        <div class="col-md-12 order-0">
            <div class="p-4">
                ...
                <table class="table border">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="fw-bold text-center">Edit Date</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($data as $val)
                        <tr>
                            <td class="p-0 m-0 text-center">{{ $loop->iteration }}</td>
                            <td class="p-0 m-0 text-center">{{ \Carbon\Carbon::parse($val->expected_date)->format('d.m.Y') }}</td>
                            <td class="p-0 m-0 text-center">
                                <input type="date" data-id="{{ $val->id }}" data-original="{{ $val->expected_date }}" class="form-control edit-date" />
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
                ...
                <button id="saveChanges" class="btn btn-primary mt-3">Update</button>
...
<script>
    ...
    document.querySelectorAll('.edit-date').forEach(input => {
        input.addEventListener('change', function() {
            ...
            if (newDate && newDate !== originalDate) {
                const existingIndex = updatedRecords.findIndex(record => record.id == id);
                
                if (existingIndex > -1) {
                    updatedRecords[existingIndex].date = newDate;
                } else {
                    updatedRecords.push({ id: id, date: newDate });
                }
            }
        });
    });
    ...
    document.getElementById('saveChanges').addEventListener('click', function() {
        ...
        if (updatedRecords.length > 0) {
            this.disabled = true;
            fetch(`/admin/periodical_dispatch_update`, {
                method: 'POST',
                headers: {
            ...
            .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        ...
                        window.location.href = '/admin/periodical-dispatch';
