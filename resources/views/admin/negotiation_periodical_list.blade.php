                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center justify-content-between">
                            <h3 class="mb-0 bc-title">
                                <b>Negotiation Periodical List</b>
                            </h3>
                                            ...
                                            <tr>
                                                <td class="sorting_1">
                                                    <div class="form-check custom-checkbox">
                                                        <input type="checkbox" class="form-check-input periodicalitem" id="periodicalitem{{ $val->id }}" value="{{ $val->id }}" data-book-id="{{ $val->id }}" required="">
                                                        <label class="form-check-label" for="periodicalitem{{ $val->id }}"></label>
                                                    </div>
                                                </td>
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        setTimeout(function() {
                            window.location.href = "/admin/negotiation_periodical_list";
                        }, 3000);
...
<script>
    $(document).ready(function() {
        ...
        $("#submitbutton11").click(function() {
            ...
            $.ajax({
                url: '/admin/sendnegotiation_periodical',
                method: 'POST',
                data: {
                    dataId: dataId,
                    _token: csrfToken
...
<script>
    $(document).ready(function() {
        $("#submitbutton").click(function() {
            ...
            var checkedBooks = $('.periodicalitem:checked');
            ...
            // Get the book IDs
            var periodicalIds = checkedBooks.map(function() {
                return $(this).data('book-id');
            }).get();

            var requestData = {
                periodicalId: periodicalIds,
            };
            ...
            $.ajax({
                url: '/admin/multisendperiodicalnegotiation',
                method: 'POST',
                data: requestData,
                success: function(response) {
