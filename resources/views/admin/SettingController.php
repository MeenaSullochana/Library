...
class SettingController extends Controller
{
    ...
    public function magazineorder_down(Request $request)
    {
        ...
        if ($request->librarytype != null &&  $request->district == null) {
            ...
            $orders = Ordermagazine::where('libraryType', '=', $request->librarytype)->where('status', '=', '0')->get();
    ...
    {
        $maga = Ordermagazine::whereIn('librarianId', function ($query) {
        ...
        foreach ($maga as $val) {
            ...
            foreach ($maga1 as $val1) {
                $count += (float) $val1->magazine_price * (float) $val1->quantity;
            }

            if (round((float) $count) == round((float) $val->totalPurchased)) {
                $obj = [
                    "library id" => $val->libraryid,
    ...
    //                     'library_name' => $librarian->libraryName,
    //                     'library_type' => $librarian->libraryType,
    //                     'received' => $received,
    ...
    //                 'Librarian Name' => $val['library_name'],
    //                 'Library Type' => $val['library_type'],
    //                 'Total Received' => $val['received'],
    //                 'Total Not Received' => $val['not_received'],
    //             ];
    //         }
    
    //         // Adding empty rows for spacing in the CSV
    //         $emptyRow = array_fill_keys(array_keys($csvData[0]), '');
    //         $csvData[] = $emptyRow;
    //         $csvData[] = $emptyRow;
    
    //         $csvContent = "\xEF\xBB\xBF"; // UTF-8 BOM
    //         $csvContent .= "S.No, Magazine Name, Periodicity, Expected Date, Librarian Name, Library Type, Total Received, Total Not Received\n";
...
public function Dispatch_libraryreport(Request $req)
{
    if ($req->monthyear !== null && $req->monthyear1 !== null) {
        ...
        // Ordering
        $query->orderBy('magazine_name', 'desc')->orderBy('expected_date', 'asc');
        $dispatches = $query->get();
        ...
        // Collect library IDs
        $libraryIds = collect($dispatches)->flatMap(function ($dispatch) {
            return json_decode($dispatch->library_id, true);
        })->unique()->all();
        ...
        foreach ($dispatches as $dispatch) {
            ...
            $magazine = Magazine::find($dispatch->magazine_id);
            ...
            foreach ($libraryIds as $libraryId) {
                ...
                $data[$libraryId]['dispatches'][] = [
                    'magazine_name' => $dispatch->magazine_name,
                    'periodicity' => $dispatch->periodicity,
                    'single_issue_rate' => $magazine->single_issue_rate,
                    'discount' => $magazine->discount,
                    'single_issue_after_discount' => $magazine->single_issue_after_discount,
                    'expected_date' => $dispatch->expected_date,
                    'received' => $received,
                    'not_received' => $notReceived,
                ];
    ...
    //     // Process each librarian record
    //     echo $librarian->librarianId . "\n"; // Assuming the Librarian model has a 'name' attribute
    // }
    //    dd(count( $diff));
...
public function Dispatch_recreate_report(Request $req)
{
    $orders = Ordermagazine::where('status', '0')->get();
     $magazine = Magazine::find("034a057d-498f-4ba8-b749-0284c0e61f57");
    
    if (!$magazine) {
        return response()->json(['error' => 'Magazine not found'], 404);
    }

    $magazineCounts = $orders->flatMap(function ($order) use ($magazine) {
        return collect(json_decode($order->magazineProduct, true))
            ->filter(function ($magazineProduct) {
                return "034a057d-498f-4ba8-b749-0284c0e61f57" == $magazineProduct['magazineid'];
            })
            ->map(function ($magazineProduct) use ($order, $magazine) {
                return [
                    'id' => $magazineProduct['magazineid'],
                    'title' => $magazine->title,
                    'order' => $order->id,
    ...
    ->map(function ($items) {
        return [
            'id' => $items->first()['id'], // The 'id' field remains the same for all items in the group
            'title' => $items->first()['title'], // Assuming title is the same for all items in the group
            'orders' => $items->pluck('order')->unique()->values()->toArray(),
            'librarianids' => $items->pluck('librarianid')->unique()->values()->toArray(),
        ];
    })
    ->values()
    ->toArray();
    ...
    foreach ($magazineCounts as $magazine) {
        $orders = array_merge($orders, $magazine['orders']);
        
        $librarians = array_merge($librarians, $magazine['librarianids']);
    }
  
//    return count($orders);

    $Dispatch=Dispatch::where('magazine_id','=',"034a057d-498f-4ba8-b749-0284c0e61f57")->get();
  
    foreach ($Dispatch as $val1) {
     $Dispatchdata=Dispatch::find($val1->id);
     $Dispatchdata->library_id = $librarians;
     $Dispatchdata->order_id = $orders;
//    dd($Dispatchdata);
     $Dispatchdata->save();
...
//      $Ordermagazine=Ordermagazine::where('id',"feeac235-3662-4831-a2c1-7fd02f929a3e")->first();
//     //  return $Ordermagazine;
//    $magazineProduct= json_decode($Ordermagazine->magazineProduct);
//     $rec = [];
//     $rec1 = [];
//     $rec11=[];
//     foreach ($magazineProduct as $val) {
//         if (!in_array($val->magazineid, $rec)) {
//             $rec[] = $val->magazineid; 
//             $rec1[] = $val;            
//         }else{
//             $rec11[] = $val; 
...
//             if($val->category == $val1->category){
//              $price= $price + $val->magazine_price;
//             }
//      }
 
//       $val1->balance_price = $val1->budget_price - $price;
//       $bal= $bal + $val1->budget_price - $price;
//        array_push($balanceAmoun1,$val1);
//    }
 
//     // return $balanceAmoun1;
//    $Ordermagazine->balanceAmount= json_encode($balanceAmoun1);
//    $Ordermagazine->magazineProduct= json_encode($rec1);
//    $Ordermagazine->totalBal= $bal;
//    $Ordermagazine->totalPurchased= $Ordermagazine->totalBudget - $bal;
//    $Ordermagazine->quantity= count($rec1);
// //    return  $Ordermagazine;
...
// }
      ...
      $Ordermagazine1 = Ordermagazine::where('status', '0')->get(); 
    // $Ordermagazine=Ordermagazine::where('librarianid',$data->id)->first();
    $counta=[];
    foreach($Ordermagazine1 as $Ordermagazine){
        $magazineProduct= json_decode($Ordermagazine->magazineProduct);
        ...
        if( count(  $magazineProduct)   != count($rec1)){
              $magazineProduct= json_decode($Ordermagazine->magazineProduct);
         
              array_push($counta,$Ordermagazine->id );
        $balanceAmount= json_decode($Ordermagazine->balanceAmount);
        $balanceAmoun1=[];
        $bal=0;
        foreach ($balanceAmount as $val1) {
       ...
       $Ordermagazine1->balanceAmount= json_encode($balanceAmoun1);
       $Ordermagazine1->magazineProduct= json_encode($rec1);
       $Ordermagazine1->totalBal= $bal;
       $Ordermagazine1->totalPurchased= $Ordermagazine->totalBudget - $bal;
       $Ordermagazine1->quantity= count($rec1);
...
//     // $mergedCollection = Publisher::all();
//     $mergedCollection = Distributor::all();
...
public function vendorwise_bookreport()
{
    ...
    foreach ($mergedCollection as $val) {
        ...
        $bookData = Book::selectRaw("
                    ...
                    SUM(CASE WHEN book_reviewer_id IS NOT NULL AND (book_status IS NULL OR book_status IN ('2', '3')) THEN 1 ELSE 0 END) as metaBookPending,
                    SUM(CASE WHEN book_reviewer_id IS NOT NULL AND book_status IN ('0', '1') THEN 1 ELSE 0 END) as metaBooksCompleted
                ")
        ...
        $finaldata[] = (object)[
            ...
            'User Type' => $type,
            'Total Books' => $bookData->totalBooks,
            'Paid Books' => $bookData->paidBooks,
            'Not Paid Books' => $bookData->notPaidBooks,
            'Meta Complete Books' => $bookData->metaBooksCompleted,
            'Meta Pending Books' => $bookData->metaBookPending,
            'Qualified Books' => $qualifiedCount,
            'Not Qualified Books' => $notQualifiedCount,
        ];
    }
