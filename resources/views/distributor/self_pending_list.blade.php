...
$categori = DB::table('books as b')
    ...
    ->leftJoin('self_nominated_books as s', function ($join) use ($distributorId) {
                                                ...
                                                <td data-label="Negotiation">
                                                    <div class="col-sm-12 m-b30">
                                                        <select class="col-sm-12 m-b30" name="user_approval" data-id="{{ $val->id }}" data-price="{{$val->price}}" data-discount="{{$val->discount}}" data-disprice="{{$val->discountedprice}}" data-calprice="{{ $val->calculated_price }}">
                                                            ...
                                                            <option style="color: red;">Quotation</option>
                                                        </select>
                                                    </div>
                                                </td>
...
<script>
    $(document).ready(function() {
        ...
        $("#submitbutton11").click(function() {
            ...
            $.ajax({
                url: '/distributor/sendselfstatus',
                method: 'POST',
                data: {
                    bookId: bookId,
                    status: status
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        setTimeout(function() {
                            window.location.href = "/distributor/self_pending_list";
                        }, 3000);
...
<script>
    $(document).ready(function() {
        $("#submitbutton").click(function() {
            ...
            $.ajax({
                url: '/distributor/sendselfamount',
                method: 'POST',
                data: data,
                success: function(response) {
                    if (response.success) {
