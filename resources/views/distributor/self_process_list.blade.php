                            ...
                            <div class="table-responsive active-projects style-1 ItemsCheckboxSec shorting ">
                                ...
                                <div id="empoloyees-tbl3_wrapper" class="dataTables_wrapper no-footer">
                                    <table id="example3" class="table dataTable no-footer" role="grid" aria-describedby="empoloyees-tbl3_info">
                                        ...
                                        <tbody>
                                        @php
                                        $distributorId = Auth::guard('distributor')->user()->id;

$categori = DB::table('books as b')
    ->join('self_nominated_books as s', function ($join) use ($distributorId) {
        $join->on('b.id', '=', 's.book_id')
             ->where('s.distributed_by', '=', $distributorId)
             ->where('s.status', '=', 0);
    })
    ->where('b.marks', '>=', 40)
    ->where('b.self_nominated', '=', 1)
    ->where('b.negotiation_status', '=', 0)
    ->select('b.*', 's.*')
    ->get();
                                                 @endphp
                                            ...
                                            <tr role="row" class="odd">
                                                ...
                                                <td data-label="Admin Price">
                                                    @if(!is_null($val->quotation_percentage))
                                                    <span><a href="#" >{{$val->quotation_percentage}}%</a> </span>
                                                    @else
                                                    <span>N/A</span>
                                                    @endif
                                                </td>
                                                <td data-label="Negotiation Price">
                                                    @if(!is_null($val->quotation_price))
                                                    <span><a href="#" >Rs {{$val->quotation_price}}</a> </span>
                                                    @else
                                                    <span>N/A</span>
                                                    @endif
                                                </td>
                                                     <td data-label="Negotiation Message">
                                                   <button type="button" id="successButton11" class="btn btn-primary btn-sm" data-id="{{$val->quotation_reason}}">View</button>
                                                  </td>
