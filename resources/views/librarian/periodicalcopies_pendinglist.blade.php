...
$('#example3').on('change', "select[name='user_approval']", function(e) {
    ...
    if (approval_ == "Received") {
        ...
        var periodicalid = $(this).data('periodicalid');


        $('#hiddenInput1').val(id);
        $('#hiddenInput2').val(libraryname);
        $('#hiddenInput3').val(periodicalid);
...
$(document).on('click', '#submitButton1', function(e) {
    ...
    $.ajax({
        type: "post",
        dataType: "json",
        url: '/librarian/periodicalcopiesstatus',
        data: data,
